...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument("-i", "--in_path", type=str, default="", help="Input fold path")
    ...
    parser.add_argument(
        "--do-evalutation", action="store_true"
    )
...
def main():
    ...
    fold_path = "/homes/gcasari/bigbrain/work_data/crops_datasets/test_folds"

    # Given the overall fold folder, call the inference for each fold
    folds = os.listdir(fold_path)
    for f in folds:

        fold_in = os.path.join(fold_path, f, "low")
        fold_out = os.path.join(args.out_path, f)

        sampler.inference(in_path=fold_in, out_path=fold_out, bs=args.bs)
